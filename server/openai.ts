...
export async function analyzeResume(request: ResumeAnalysisRequest): Promise<ResumeAnalysisResponse> {
  ...
  try {
    const response = await openai.chat.completions.create({
      ...
      messages: [
        { role: "system", content: "You are an expert resume analyst with deep knowledge of recruitment, applicant tracking systems, and industry-specific requirements." },
    ...
    const result = JSON.parse(content) as ResumeAnalysisResponse;
    return result;
  } catch (error: any) {
    console.error("Error analyzing resume with OpenAI:", error.message);
