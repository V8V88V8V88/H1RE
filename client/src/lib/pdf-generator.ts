...
export function generatePDFReport(result: ResumeAnalysisResponse, resumeText?: string): void {
  const {
    overallScore,
    grammarScore,
    atsScore,
  ...
  addText('Generated by H1RE', 105, 285, { align: 'center' });
  
  // Extract name from resume text for filename
  const extractedName = resumeText ? extractNameFromResume(resumeText) : "Resume";
  const sanitizedName = extractedName.replace(/[^a-zA-Z0-9-]/g, '').substring(0, 50);
  const filename = `${sanitizedName}-Resume-Analysis-Report.pdf`;
